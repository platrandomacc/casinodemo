  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        className="flex w-full items-center justify-between rounded-full border border-transparent hover:border-sky-500/25 hover:border-emerald-500/40 bg-[#171A22] px-4 py-2 text-left text-sm text-[#F7F7F8] transition hover:border-[#22C55E]/30"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <span>
          <span className="block text-xs uppercase tracking-[0.28em] text-[#8D95A8]">{label}</span>
          <span className="mt-1 block font-medium text-white">{selectedOption.label}</span>
        </span>
